...
Die Verteilung der Flächengrössen war stark rechtsschief. Wie in @fig-plot3 sichtbar, ist dies auf wenige Individuen mit aussergewöhnlich grossen Home Ranges zurückzuführen. sichtbar, ist dies auf wenige Individuen mit aussergewöhnlich grossen Home Ranges zurückzuführen.
...
Die Aktivitätsmuster zeigen, dass die untersuchten Katzen insbesondere während der Abenddämmerung und der Nacht aktiv sind. Dieses Muster entspricht dem häufig beschriebenen dämmerungs- und nachtaktiven Verhalten von Katzen (Merčnik et al., 2023). Die verwendeten Bewegungsparameter Geschwindigkeit und zurückgelegte Distanz stellen zentrale Grössen der CMA dar und ermöglichen eine quantitative Analyse des Bewegungsverhaltens der Katzen aus Ås (Dodge et al., 2008). 

Gleichzeitig verdeutlichen die Resultate die von Laube (2014) beschriebene Problematik des „Semantic Gap“. Obwohl erhöhte Geschwindigkeiten und grössere Distanzen auf höhere Aktivität hindeuten, kann aus den Bewegungsdaten nicht eindeutig auf das tatsächliche Verhalten geschlossen werden. Eine erhöhte Geschwindigkeit kann beispielsweise Jagdverhalten, Erkundung oder einen Ortswechsel widerspiegeln. Dies wird auch von Shamoun-Baranes et al. (2012) betont.

Der verwendete Schwellenwert von 5 Minuten für zulässige Zeitabstände stellt einen methodischen Kompromiss zwischen Datenqualität und Datenverlust dar. Zwar reduziert er das Problem des „Granularity Grief“, kurzfristige Bewegungen können jedoch weiterhin verpasst oder unterschätzt werden (Laube und Purves, 2011). Bei grossen Zeitabständen zwischen Messungen bleibt unklar, ob sich eine Katze konstant bewegte oder zwischen schnellen und langsamen Bewegungsphasen wechselte.

Die Analyse der Home Ranges zeigt deutliche Unterschiede zwischen urbanen und ländlichen Katzen. Die Einteilung in urban und ländlich ist in diesem Datensatz jedoch nur eingeschränkt interpretierbar, da das Untersuchungsgebiet insgesamt ländlich geprägt ist. Dennoch liefert sie Hinweise darauf, dass Landschaftsstruktur und Raumnutzung bei zukünftigen Analysen genauer berücksichtigt werden sollten.

Die Untersuchung raumzeitlicher Begegnungen stellt einen weiteren Schritt von der reinen Bewegungsanalyse zur Interpretation möglicher sozialer Interaktionen dar. Dennoch erlaubt die gewählte Begegnungsdefinition keine Aussage über tatsächliche soziale Kontakte. Die Analyse identifiziert lediglich räumliche und zeitliche Nähe zwischen Individuen. Die Resultate deuten darauf hin, dass die Beziehung zwischen räumlicher Überlappung und Begegnungshäufigkeit nicht linear ist. Ob es sich dabei um direkte Kontakte, soziale Beziehungen oder zufällige Näheereignisse handelt, kann anhand der GPS-Daten nicht beurteilt werden (Dodge et al., 2013).

Insgesamt zeigen die Ergebnisse sowohl die Möglichkeiten als auch die Grenzen GPS-basierter Bewegungsanalysen. Die Kombination aus Aktivitäts-, Home-Range- und Begegnungsanalysen erlaubt eine differenzierte Betrachtung der Bewegungsmuster von Katzen. Für eine weiterführende biologische Interpretation wären jedoch GPS-Daten ohne räumlichen Versatz, kürzere Messintervalle und zusätzliche Verhaltensinformationen notwendig.
...
Schliesslich schränkt der aus Datenschutzgründen angewendete räumliche Versatz der Koordinaten die exakte räumliche Lokalisierung ein. Punktgenaue Interpretationen einzelner Begegnungsorte sind dadurch limitiert, während aggregierte Auswertungen, etwa zur allgemeinen Landschaftsklassifikation, weiterhin möglich sind.

# Literaturverzeichnis


```{r}
wordcountaddin::word_count("Semesterproject_final.qmd")
```
