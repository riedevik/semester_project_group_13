---
title: Muster in Bewegungsdaten von Hauskatzen
subtitle: Semesterprojekt Gruppe 13
...
Katzen gelten als überwiegend dämmerungs- und nachtaktive Tiere, deren Aktivität stark von Lichtbedingungen beeinflusst wird [@mercnik2023]. Sie werden oft als territorial eingeschätzt; die Literatur zeigt jedoch, dass territoriales Verhalten kontextabhängig ist und nicht bei allen Katzen gleich ausgeprägt auftritt [@crowell2004]. 

Das Konzept der Home Range beschreibt den regelmässig genutzten Lebensraum eines Tieres zur Nutzung essenzieller Ressourcen [@powell2012]. GPS-basierte Methoden erlauben präzise Schätzungen, etwa mittels der Kernel Density-Methode [@worton1989], welche in dieser Arbeit verwendet wird.

# Material und Methoden

## Daten

Für die Analyse wurde der öffentlich auf GitHub verfügbare Datensatz **[CatTrack_public](https://github.com/richbi/CatTrack_public)** verwendet, der GPS-Daten von 92 Katzen im Raum Ås, nahe Oslo, enthält. Die Daten umfassen Zeitstempel sowie räumliche Positionen der Katzen während des Monats Mai 2021. Aus Datenschutzgründen sind die Koordinaten mit einem unbekannten räumlichen Versatz versehen, um die Anonymität der Katzenbesitzer*innen zu wahren.

Die GPS-Katzendaten entsprechen der Lagrange-Perspektive, welche es ermöglicht, Individuen kontinuierlich zu verfolgen. Sie erlaubt die Analyse individueller Bewegungsparameter wie Distanz, Geschwindigkeit und Aufenthaltsgebiete [@laube2017].

Für die Unterteilung der Tageszeiten wurden zusätzlich [tages- und ortspezifische  Informationen](https://www.timeanddate.com/sun/norway/oslo?month=5&year=2021) zu Sonnenauf- und untergang sowie den Morgen- und Abenddämmerungszeiten integriert. Um die Home Ranges nach Katzentyp (urban/ländlich) unterscheiden zu können, wurden Daten aus [OpenStreetMap](https://download.geofabrik.de/europe/norway.html?utm) zur Ableitung urbaner und ländlicher Landnutzungskategorien hinzugezogen.
...
Für die Analyse wurden **Geschwindigkeit** und **zurückgelegte Distanz** als Proxies für Aktivität definiert. Beide sind zentrale Bewegungsparameter der CMA [@dodge2008]. Die GPS-Daten wurden mit Sonnenauf- und untergang sowie Dämmerungsphasen verknüpft und jedem GPS-Punkt eine Tageszeitkategorie, basierend auf biologischen Lichtverhältnissen, zugewiesen:
...
Die Analyse der Geschwindigkeit und zurückgelegten Distanz als Proxy für die Aktivität zeigt Unterschiede zwischen den Tageszeiten. Auf Basis der katzenspezifischen Mittelwerte waren die untersuchten Katzen während der Nacht und der Abenddämmerung am aktivsten (siehe @fig-plot1). Die höchsten mittleren Geschwindigkeiten waren nachts (0.087 m/s) und während der Abenddämmerung (0.086 m/s), während sie tagsüber bei 0.074 m/s und in der Morgendämmerung bei 0.071 m/s lagen. Ein vergleichbares Muster zeigt sich bei der zurückgelegten Distanz pro Stunde (Nacht: 313 m/h, Abenddämmerung: 310 m/h, Tag: 267 m/h, Morgendämmerung: 255 m/h). Gleichzeitig zeigt sich eine hohe individuelle Variabilität zwischen den Katzen (@fig-plot1).
...
Die Ergebnisse zeigen deutliche Unterschiede in der Grösse der Home Ranges (KDE 95%) zwischen den untersuchten Katzen (Min. 0.2 ha, Median 1.5 ha, Max. 377 ha). Die Sensitivitätsanalyse mit der 80 %-KDE führte erwartungsgemäss zu kleineren Flächenwerten, bestätigte jedoch die grundsätzlichen Muster der Analyse (siehe @fig-plot2). 
...
```{r}
#| label: fig-plot2
#| fig-cap: 'Kernel-Density-basierte Home-Range-Schätzung der 92 Katzen. Links: 95 %-KDE Home Ranges; rechts: 80 %-KDE Kernbereiche.'
...
Die Verteilung der Flächengrössen war stark rechtsschief. Wie in @fig-plot3 sichtbar, ist dies auf wenige Individuen mit aussergewöhnlich grossen Home Ranges zurückzuführen.
...
Der Vergleich zwischen urbanen und ländlichen Katzen deutet darauf hin, dass ländliche Katzen tendenziell grössere Home Ranges und eine stärkere Variabilität aufweisen als urbane Katzen. Urbane Katzen nutzten dagegen kleinere und räumlich stärker konzentrierte Gebiete (siehe @fig-plot4).
...
```{r}
#| label: fig-plot4
#| fig-cap: 'Vergleich der Home-Range-Grössen zwischen urbanen und ländlichen Katzen. Links: 95 %-KDE Home-Range-Grösse nach dominantem Landtyp innerhalb der Home Range; die y-Achse ist logarithmisch skaliert. Rechts: Räumliche Darstellung der als urban oder ländlich klassifizierten Home Ranges.'
...
Die räumliche Analyse zeigt zudem, dass die Anzahl der Begegnungen mit zunehmender Überlappung zunächst ansteigt, jedoch bei mittlerer Überlappung von 3 Home Ranges ein Maximum erreicht und ab 4 Überlappungen wieder abnimmt (siehe @fig-plot6, rechts). Insgesamt deuten die Ergebnisse darauf hin, dass räumliche Nähe und gemeinsame Raumnutzung die Wahrscheinlichkeit von Begegnungen erhöhen, die Verteilung der Begegnungen jedoch stark heterogen über die Katzenpaare ist (siehe @fig-plot6, links).
...
```{r}
#| label: fig-plot6
#| fig-cap: 'Räumliche Verteilung der Begegnungen und Zusammenhang mit überlappenden Home Ranges. Links: Begegnungspunkte innerhalb der 95 %-KDE Home Ranges. Rechts: Anzahl Begegnungen in Abhängigkeit von der Anzahl überlappender Home Ranges.'
...
Die Aktivitätsmuster zeigen, dass die untersuchten Katzen insbesondere während der Abenddämmerung und der Nacht aktiv sind. Dieses Muster entspricht dem häufig beschriebenen dämmerungs- und nachtaktiven Verhalten von Katzen [@mercnik2023]. Die verwendeten Bewegungsparameter Geschwindigkeit und zurückgelegte Distanz stellen zentrale Grössen der CMA dar und ermöglichen eine quantitative Analyse des Bewegungsverhaltens der Katzen aus Ås [@dodge2008]. 

Gleichzeitig verdeutlichen die Resultate die von @laube2014 beschriebene Problematik des „Semantic Gap“. Obwohl erhöhte Geschwindigkeiten und grössere Distanzen auf höhere Aktivität hindeuten, kann aus den Bewegungsdaten nicht eindeutig auf das tatsächliche Verhalten geschlossen werden. 

Der verwendete Schwellenwert von 5 Minuten für zulässige Zeitabstände stellt einen methodischen Kompromiss zwischen Datenqualität und Datenverlust dar. Zwar reduziert er das Problem des „Granularity Grief“, kurzfristige Bewegungen können jedoch weiterhin verpasst oder unterschätzt werden, da die zeitliche Auflösung der GPS-Daten die berechneten Bewegungsparameter beeinflusst [@laube2011]. Ein Beispiel dafür wären schnelle und langsame Bewegungen wie das Jagdverhalten. 

Die Analyse der Home Ranges zeigt deutliche Unterschiede zwischen urbanen und ländlichen Katzen. Die Einteilung in urban und ländlich ist in diesem Datensatz jedoch nur eingeschränkt interpretierbar, da das Untersuchungsgebiet insgesamt ländlich geprägt ist. Dennoch liefert sie Hinweise darauf, dass Landschaftsstruktur und Raumnutzung bei zukünftigen Analysen genauer berücksichtigt werden sollten. Schliesslich schränkt der aus Datenschutzgründen angewendete räumliche Versatz der Koordinaten die exakte räumliche Lokalisierung ein. Punktgenaue Interpretationen einzelner Begegnungsorte sind dadurch limitiert, während aggregierte Auswertungen, etwa zu allgemeinen Landschaftstypen, weiterhin möglich sind.

Die Untersuchung raumzeitlicher Begegnungen stellt einen weiteren Schritt von der reinen Bewegungsanalyse zur Interpretation möglicher sozialer Interaktionen dar. Dennoch erlaubt die gewählte Begegnungsdefinition keine Aussage über tatsächliche soziale Kontakte. Die Analyse identifiziert lediglich räumliche und zeitliche Nähe zwischen Individuen. Die Schwellenwerte von 10 Metern und 60 Sekunden stellen dabei einen pragmatischen Kompromiss zwischen biologischer Plausibilität und Datenqualität dar [@long2014]. Die Resultate deuten darauf hin, dass die Beziehung zwischen räumlicher Überlappung und Begegnungshäufigkeit nicht linear ist. Ob es sich dabei um direkte Kontakte, soziale Beziehungen oder zufällige Näheereignisse handelt, kann anhand der GPS-Daten nicht abschliessend beurteilt werden [@dodge2013].

Insgesamt zeigen die Ergebnisse sowohl die Möglichkeiten als auch die Grenzen GPS-basierter Bewegungsanalysen. Die Kombination aus Aktivitäts-, Home-Range- und Begegnungsanalysen erlaubt eine differenzierte Betrachtung der Bewegungsmuster von Katzen. Für eine weiterführende Interpretation des Katzenverhaltens wären jedoch GPS-Daten ohne räumlichen Versatz, kürzere Messintervalle und zusätzliche Verhaltensinformationen notwendig.

# Verwendung von KI

**NotebookLMM** [@notebooklm] wurde für die selektive Ausgabe und Zusammenfassung von Inhalten aus wissenschaftlichen Papers verwendet.

**ChatGPT** [@ChatGPT] wurde unterstützend für Recherche, sprachliche Überarbeitung als auch als Hilfe beim Code verwendet.

```{r}
words <- wordcountaddin::word_count("Semesterproject_final.qmd")
cat("Anzahl Wörter:", words, "/ Anzahl Zeichen (inkl. Leerzeichen und Literaturverzeichnis): 19'088")
```
